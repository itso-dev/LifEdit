...
include_once('../../head.php');
    if (!empty($_GET['id'])) {
        ...
        echo "alert('잘못된 접근입니다.'); location.href='news.php'";
        echo "</script>";    }

    $board_sql = "select * from news_tbl where id = $id";
    $board_stt = $db_conn->prepare($board_sql);
    $board_stt->execute();
    $board = $board_stt->fetch();

    //View Count
    $view_sql = "UPDATE news_tbl set view_cnt = view_cnt + 1 where id = " .$id;
    $viewStmt = $db_conn->prepare($view_sql);
    $viewStmt->execute();

    //Prev Board
    $prev_sql = "select * from news_tbl where id < " .$id ." order by id desc limit 1";
    $prev_stt = $db_conn->prepare($prev_sql);
    $prev_stt->execute();
    $prev = $prev_stt->fetch();
    if($prev){
        $prev_title = $prev['title'];
        $prev_href = "news-detail.php?id={$prev['id']}";
    }
    //Next Board
    $next_sql = "select * from news_tbl where id > " .$id ." order by id ASC limit 1";
    $next_stt = $db_conn->prepare($next_sql);
    $next_stt->execute();
    $next = $next_stt->fetch();
    if($next){
        $next_title = $next['title'];
        $next_href = "news-detail.php?id={$next['id']}";
    }
?>
