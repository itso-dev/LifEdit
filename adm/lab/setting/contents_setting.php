...
//이전 파일
$old_img_file0 = $_POST['old_img_file0'];
...
//대표 이미지
if (isset($_FILES['img_file0']) && $_FILES['img_file0']['name'] != "") {
     $img_file0 = $_FILES['img_file0'];
     $ext1 = substr($img_file0['name'], strrpos($img_file0['name'], '.') + 1);
     ...
     // 파일 크기 체크
     if ($img_file0['size'] >= $max_file_size) {
          echo "<script type='text/javascript'>";
          echo "alert('5MB 이하의 파일만 업로드 가능합니다.'); history.back()";
          echo "</script>";
     }
     //파일명 변경
     $img_chg_file0 = date("Y-m-d H:i:s:u") .$img_file0['name'];
     $img_chg_file0 = str_replace(' ', '', $img_chg_file0);
     if (move_uploaded_file($img_file0['tmp_name'], $upload_directory . $img_chg_file0)) {
          $img_file0_sql = " img_file_name0 = '".$img_file0['name']."',
           img_chg_name0 = '$img_chg_file0',";
     }

     unlink($upload_directory. $old_img_file0);
...
$modify_sql = "update contents2_tbl
          set 
          ".$img_file0_sql."
          ".$img_file1_sql."
          ".$img_file2_sql."
...
echo "<script type='text/javascript'>";
echo "alert('수정을 완료했습니다.'); location.href='../$type.php?menu=77'";
echo "</script>";
