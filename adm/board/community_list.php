...
<link rel="stylesheet" type="text/css" href="<?= $site_url ?>/css/board_list.css" rel="stylesheet" />
    ...
    <div class="search-container">
        <form method="post">
            ...
            <input type="text" name="skey" placeholder="제목 / 작성자를 입력해주세요.">
            <input type="submit" value="검색"/>
        </form>
    </div>
    <form name="fmemberlist" id="fmemberlist" action="./setting/board_delete.php" method="post">
        <input type="hidden" name="type" value="community"/>
        <div class="btn_fixed_top">
            <span onclick="delData()" class="btn btn_02">선택삭제</span>
            <span onclick="noticeData()" class="btn btn_02">공지등록</span>
            <a href="community_form.php?menu=44&type=insert" id="member_add" class="btn btn_01">게시글 추가</a>
        </div>
        <div class="card-body">
                ...
                <table class="table border-bottom" style="min-width: 800px;">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" id="mb_list_id" width="10%" class="text-center">작성일</th>
                            <th scope="col" id="mb_list_id" width="10%" class="text-center">구분</th>
                            <th scope="col" id="mb_list_name" width="10%" class="text-center">작성자</th>
                            <th scope="col" id="mb_list_join" width="40%" class="text-center">제목</th>
                            <th scope="col" id="mb_list_join" width="10%" class="text-center">조회수</th>
                        ...
                        <tbody>
                            ...
                            <tr class="bg0">
                                ...
                                <td headers="mb_list_name" class="td_mbname text-center"><? if($list_row['is_notice'] == 'Y') echo "공지";  ?></td>
                                <td headers="mb_list_name" class="td_mbname text-center"><?=$list_row['writer']?></td>
                                <td headers="mb_list_join" class="td_date text-center"><?=$list_row['title']?></td>
                                <td headers="mb_list_join" class="td_date text-center"><?=$list_row['view_cnt']?></td>
                                <td headers="mb_list_mng" class="td_mng td_mng_s text-center"><a href="community_form.php?menu=44&type=modify&id=<?=$list_row['id']?>" class="btn btn_03">수정</a></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
...
function delData(){
    ...
    if( count == 0) {
        ...
        $("#fmemberlist").attr("action", "./setting/board_delete.php");
        $("#fmemberlist").submit();
        return true;
    }
}
...
function noticeData(){
    ...
    if( count == 0) {
        alert("공지로 등록할 대상을 선택해주세요.");
        return false;
    } else {
        $("#fmemberlist").attr("action", "./setting/community_set_notice.php");
        $("#fmemberlist").submit();
        return true;
    }
}
